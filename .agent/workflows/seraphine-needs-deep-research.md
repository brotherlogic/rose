# 🔬 The `rose-needs-deep-research` Label Workflow

When an issue is labeled with `rose-needs-deep-research`, the AI assistant is triggered to perform comprehensive research on the problem space before moving into requirements gathering.
...
* **Error States:**
  * **No Viable Options Found:** If extensive research yields no viable options, document the findings and the blockers. Post this as a comment and do NOT proceed to create a requirements sub-issue. Remove the `rose-needs-deep-research` label and stop execution.
  * **Ambiguous Problem Space:** If the problem space is too broad or ambiguous to yield concrete options, comment with clarifying questions, remove the `rose-needs-deep-research` label, and stop execution.
